...
export default function Home() {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    emailjs
      .send("service_rucugra", "template_fdm69q8", {
        sender: formemail,
      })
      .then(
        (response) => {
          emailjs
            .send("service_rucugra", "template_88gujw2", {
              email: formemail,
            ...
            .then(
              (response) => {
                ...
                setLoading(false);
                alert(
                  "You have been placed on the waitlist. A confirmation email has been sent to you."
                );
              },
              (error) => {
                setLoading(false);
      ...
      <div className="text-white home">
        <div className="h-full text-center flex flex-col items-center justify-start mt-16 w-screen gap-3 lg:gap-4">
          ...
          >
            ...
            <div className="flex items-center w-4/5 lg:w-auto">
              <input
                type="email"
          ...
          <TweetEmbed
            tweetId="1814971983307690127"
            options={{ theme: "dark" }}
            className="w-4/5 lg:w-1/2 twitter-embed"
          />
        </div>
      </div>
    </>
